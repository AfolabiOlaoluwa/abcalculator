...
def calculate(request):
    ...
    final_pvalue = round(pvalue,3)
    #last_result = LastResult(visitors_control=visitors_control,visitors_variant=visitors_variant,conversion_control=conversion_control,conversion_variant=conversion_variant,pvalue=final_pvalue)
...
#     """
#     data = serializers.serialize('json',LastResult.objects.latest('created'))
...
def p_value(visit_control, conv_control, visit_var,conv_var):
    ...
    than 0.05 """
    if visit_control < conv_control or visit_var < conv_var:
        raise ValueError('The size numbers must be greater than the number of conversions')

    mean_visit = conv_control / visit_control
    mean_conv = conv_var / visit_var
